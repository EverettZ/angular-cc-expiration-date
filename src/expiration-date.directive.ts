...
export class ExpirationDateDirective {
  ...
  constructor(private el: ElementRef, private _expService: ExpirationDateService) {
    this.target = this.el.nativeElement;
  }

  @HostListener('keypress', ['$event']) onKeypress(e) {
    if (this._expService.restrictNumeric(e)) {
      if (this._expService.restrictExpiry(e.which, this.target)) {
        this.formatExpiry(e);
  ...
  @HostListener('keydown', ['$event']) onKeydown(e) {
    if (this._expService.restrictNumeric(e) && this._expService.restrictExpiry(e.which, this.target)) {
  ...
  private reformatExpiry(e) {
    setTimeout(() => {
      ...
      val = this._expService.replaceFullWidthChars(val);
      val = this._expService.formatExpiry(val);
      this.target.selectionStart = this.target.selectionEnd = this._expService.safeVal(val, this.target);
    });
  }
